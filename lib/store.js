...
async function loadStore() {
  ...
  await ensureSchema();
  const sql = getSql();
  const brothers = await sql`select * from brothers order by name asc`;
  const sessions = await sql`select * from sessions order by datetime desc`;
  const attendance = await sql`select session_id, brother_id from session_attendance`;
  const visitors = await sql`select * from session_visitors order by name asc`;
...
async function createBrother(input) {
  ...
  await sql`
    insert into brothers (
      id, name, address, phone, birth_date, cim, degree,
      initiation_date, elevation_date, exaltation_date,
      emerito_date, benemerito_date, wife_name, wife_birth_date, wife_phone
    ) values (
      ${brother.id}, ${brother.name}, ${brother.address}, ${brother.phone}, ${brother.birthDate}, ${brother.cim}, ${brother.degree},
